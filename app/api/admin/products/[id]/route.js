...
import { archiveProduct, getProductById, saveProduct } from '@/lib/backoffice/repository';
...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const previous = await getProductById(params.id);
    if (!previous) return json({ error: 'Product not found.' }, { status: 404 });
    const product = await saveProduct({ ...previous, ...(await request.json()), id: params.id }, { actor: auth.admin });
    return json({ product });
  } catch (error) {
...
export async function DELETE(request, { params }) {
  ...
  if (auth.error) return auth.error;
  try {
    const product = await archiveProduct(params.id, auth.admin);

...
export const productSchema = z.object({
  ...
  size: z.array(z.string().trim()).default([]),
  color: z.array(z.string().trim()).default([]),
...
export const productInputSchema = z.object({
  ...
  size: z.union([z.array(z.string()), z.string()]).optional(),
...
export const cmsPageInputSchema = z.object({
  id: z.string().trim().optional(),
  slug: z.string().trim().optional(),
  type: z.enum(['about', 'faq', 'terms', 'homepage_section', 'contact', 'navigation', 'account', 'privacy', 'workshops', 'custom']).default('custom'),
...
export function normalizeProductInput(input, previous = null) {
  ...
  const size =
    typeof parsed.size === 'string'
      ? parsed.size.split(',').map((value) => value.trim()).filter(Boolean)
      : parsed.size ?? previous?.size ?? [];
  const color =
    typeof parsed.color === 'string'
      ? parsed.color.split(',').map((value) => value.trim()).filter(Boolean)
      : parsed.color ?? previous?.color ?? [];
  const material = parsed.material ?? previous?.material ?? '';

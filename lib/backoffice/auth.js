...
export async function loginAdmin(request, body) {
  ...
  const configuredEmail = (process.env.ADMIN_EMAIL || 'owner@wildcat.local').trim().toLowerCase();
  ...
  const expectedPasswordHash = process.env.ADMIN_PASSWORD_SHA256 || passwordHash(getConfiguredPassword());
...
export function isAuthConfiguredForProduction() {
  return Boolean(process.env.ADMIN_PASSWORD || process.env.ADMIN_PASSWORD_SHA256) && Boolean(process.env.ADMIN_SESSION_SECRET);
}

export function getAuthReadiness() {
  const passwordConfigured = Boolean(process.env.ADMIN_PASSWORD || process.env.ADMIN_PASSWORD_SHA256);

...
import { getMediaReadiness } from './cloudinary';
...
export async function saveProduct(input, options = {}) {
  ...
  const product = normalizeProductInput(input, previous);
  ...
  await writeAuditEvent({
    actor: options.actor,
    action: options.action || (previous ? 'product_update' : 'product_create'),
    entityType: 'product',
    entityId: product.id,
    before: previous,
    after: product,
...
async function applyInventoryForConfirmedOrder(order, actor) {
...
export async function updateOrder(id, input, actor) {
  ...
  if (next.status !== previous.status) {
    ...
    if (['confirmed', 'paid'].includes(next.status) && !['confirmed', 'paid'].includes(previous.status)) {
      await applyInventoryForConfirmedOrder(next, actor);
    }
  }
...
export async function saveCmsPage(input, options = {}) {
  ...
  await writeAuditEvent({
    actor: options.actor,
    action: options.action || (previous ? 'cms_update' : 'cms_create'),
    entityType: 'cms_page',
    entityId: page.id,
    before: previous,
    after: page,
...
export async function getDashboardSummary() {
  await ensureSeeded();
  const [products, orders, pages, audit] = await Promise.all([
    listProducts(),
